...
public:
    ...
    virtual double Evaluate(const std::function<const CellInterface* (Position)>& functor) const = 0;
...
// При делении на 0 выбрасывайте ошибку вычисления FormulaError
    double Evaluate(const std::function<const CellInterface* (Position)>& functor) const override {
        ...
        switch (type_) {
        case Add:
            result = lhs_->Evaluate(functor) + rhs_->Evaluate(functor);
            break;
        case Subtract:
            result = lhs_->Evaluate(functor) - rhs_->Evaluate(functor);
            break;
...
public:
    ...
    double Evaluate(const std::function<const CellInterface* (Position)>& functor) const override {
        if (!cell_->IsValid()) {
            ...
            if (functor(*cell_) == nullptr) {
                return 0;
            }
            CellInterface::Value cell_value = functor(*cell_)->GetValue();
            if (std::holds_alternative<std::string>(cell_value)) {
                ...
                if (str_value.empty()) {
                    ...
                    int d_value = 0.0;
                    auto [ptr, ec] = std::from_chars(str_value.data(), str_value.data() + str_value.size(), d_value);
...
double FormulaAST::Execute(const std::function<const CellInterface* (Position)>& functor) const {
    return root_expr_->Evaluate(functor);
}
